  ...
  const renderSummaryBox = (title, data, currency = "") => {
    ...
    return (
      ...
      >
        ...
        <span
          className={`${classes.summaryDiff} ${
            data.diff > 0 ? classes.summaryDiffInc : classes.summaryDiffDec
          }`}
        >
          {data.diff > 0 ? (
  ...
  const renderTransactionRow = (transaction) => {
    ...
    return (
      <tr key={uid.v4()}>
        ...
        <td>IDR {numeral(transaction.price).format("0,0.00")}</td>
        <td>
  ...
  return (
    <div className={classes.latestTransactions}>
      <div className={classes.boxHeader}>
        <h5 className={classes.boxHeaderTitle}>Latest Transactions</h5>
        <Button className={classes.buttonWithIcon} bsSize="small">
...
const renderLatestComments = (comments) => {
  ...
  return (
    <div className={classes.latestComments}>
      <div className={classes.boxHeader}>
        <h5 className={classes.boxHeaderTitle}>Latest Comments</h5>
        <Button className={classes.buttonWithIcon} bsSize="small">
          See All Comments
          <i className="fa fa-angle-right"></i>
        </Button>
...
const renderMostViewedItems = (mostViewed) => {
  const getQuantityStatusClass = (quantity) => {
    return classNames("fa", "fa-circle", classes.mostViewedItemStatus, {
      ...
      [`${classes.mostViewedItemStatusWarning}`]:
  ...
  return (
    <div className={classes.mostViewedItems}>
      <div className={classes.boxHeader}>
        <h5 className={classes.boxHeaderTitle}>Most Viewed</h5>
        <Button className={classes.buttonWithIcon} bsSize="small">
          See All Transactions
          <i className="fa fa-angle-right"></i>
        </Button>
...
class ECommerceContainer extends RoutedComponent {
  ...
  renderChart(data) {
    ...
    const datasetName =
      data.selected === DataSet.Revenue ? "Revenue" : "Items Sold";
    ...
    return (
      <div className={classes.chart}>
        <div className={classes.boxHeader}>
          <div>
            <h5 className={classes.boxHeaderTitle}>Report Statisticts</h5>
        ...
        <Charts.HighchartBasicColumn
          className={classes.chartObject}
          config={chartData}
        />
      </div>
    );
  ...
  render() {
    return (
      <div className={classes.mainWrap}>
        <Row>
          <Col md={12}>{renderSection(renderSummary, this.state.Summary)}</Col>
        </Row>
        <Row className={classes.sectionRow}>
          <Col md={6}>{this.renderChart(this.state.Report)}</Col>
          <Col md={6}>
            {renderSection(
