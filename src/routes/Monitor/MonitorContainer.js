...
const renderCpuPanel = (cpuData) => (
  <ListGroupItem className={classes.panelGroupItem}>
    <h5>CPU</h5>
    <p>
      {cpuData.Name} <i className="fa fa-fw fa-info-circle"></i>
    </p>
    {_.map(cpuData.Cores, (core) => (
      <Row key={core._id} className="p-y-1">
        <Col sm={6}>
          Core {core.No} @{" "}
          <span className="text-white"> {core.LoadPercentage}%</span>
        </Col>
        <Col sm={6}>
          <SlimProgressBar now={core.LoadPercentage} className="m-t-1" />
        </Col>
      </Row>
    ))}
  </ListGroupItem>
...
const renderMemoryPanel = (memoryData) => (
  <ListGroupItem className={classes.panelGroupItem}>
    <h5>
      Memory <small>({memoryData.Type})</small>
    </h5>
    <p>
      {memoryData.Name} <i className="fa fa-fw fa-info-circle"></i>
    </p>
    ...
    <Media>
      ...
      <Media.Body>
        <p className={classes.totalMemoryValue}>
          {memoryData.TotalSize.toFixed(1)}{" "}
          <small className="text-white">GB</small>
        </p>
        Total Memory
      </Media.Body>
    </Media>
    ...
    <Row>
      {_.map(
        [
          {
            name: "Allocated",
            value: memoryData.Allocated,
            color: Colors.brandCerulean,
          },
          {
            name: "In Cache",
            value: memoryData.InCache,
        ...
        (memoryEntry, index) => (
          <Col sm={4} key={index}>
            ...
            <p>
              {Math.ceil(
                (memoryEntry.value / (memoryData.TotalSize * 1024)) * 100
              )}{" "}
              %
            </p>
          </Col>
...
const renderNetworkPanel = (networkData) => (
  <ListGroupItem className={classes.networkGroupItem}>
    <h5>
      Interface Traffic <small>({networkData.Interface})</small>
    </h5>
...
const renderTemperaturePanel = (tempData) => (
  <ListGroupItem className={classes.tempGroupItem} key={tempData._id}>
    <h5>
      {tempData.Name} <small>({tempData.Status})</small>
    </h5>
    <p className={classes.tempSummary}>
      ...
      <i className={`fa fa-fw fa-arrows-h ${classes.tempDeltaIcon}`}></i>{" "}
      Average
      <span className={classes.tempValue}>
        {" "}
        {tempData.AverageTemp}
        <sup>o</sup>C
      </span>
    </p>
          ...
          <Row>
            <Col xs={6}>
              <div className="small">
                <i
                  className={`fa fa-fw fa-circle ${
                    classes["dot" + volumeColors[i].colorName]
                  }`}
                ></i>
                <br />
                Used Space
              ...
              <p>{calcPercentage(volume.Size.Used, volume.Size.Total)}%</p>
            </Col>
            <Col xs={6}>
              <div className="small">
                <i
                  className={`fa fa-fw fa-circle ${
                    classes["dot" + volumeColors[i].colorName + "Dark"]
                  }`}
                ></i>
                <br />
                Free Space
              ...
              <p>{calcPercentage(volume.Size.Free, volume.Size.Total)}%</p>
            </Col>
          </Row>
        </Col>
      ))}
      ...
      {_.map(drivesData, (drive) => (
        <tr key={drive._id}>
          ...
          <td className="text-right">
            <span className={classes.driveUsedSpace}>
              {drive.Size.Used.toFixed(2)} TiB
            </span>
            <span> / </span>
          ...
          <td className="text-right">
            ...
            {drive.Status === "Healthy" ? (
              <i
                className={`fa fa-fw fa-check-circle ${classes.iconHealthy}`}
              ></i>
            ) : (
              <i
                className={`fa fa-fw fa-exclamation-circle ${classes.iconDegraded}`}
              ></i>
            )}
          </td>
        </tr>
...
class MonitorContainer extends RoutedComponent {
  ...
  simulateNetworkTraffic() {
    ...
    const prevTraffic = _.last(
      this.state.Network.Traffic,
      this.state.Network.Traffic.length - 1
    );
  ...
  render() {
    return (
      <Row>
        ...
        <Col lg={8} className={classes.volumeWrap}>
          <p>
            <Faker>{"[lorem.paragraph]"}</Faker>
          </p>
          <Divider className="m-t-3 m-b-1">Volume Status</Divider>
          {sectionRender(renderVolumesPanel, this.state.Volumes)}

          <Divider className="m-t-3 m-b-1">Mounted Drives</Divider>
